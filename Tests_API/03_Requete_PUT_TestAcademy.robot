...
Test Requete PUT
    ...
    &{Corps_Creation}=    Create Dictionary    first_name=Initial    last_name=User    email=initial.user@api.testacademy.fr
    ${Reponse_POST}=   POST    ${Base_URL}api/users    json=${Corps_Creation}    headers=${headers}    expected_status=201
    ${Id_Utilisateur}=    Get From Dictionary    ${Reponse_POST.json()}    id
    
    # Mettre à jour l'utilisateur créé
